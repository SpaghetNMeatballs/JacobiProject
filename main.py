...
class Jacobi:
    def __init__(self, a: list[list[float]]):
        ...
        self.height = len(a)
        self.s = 0.
        self.v = [[1. for _ in range(self.height)] for _ in range(self.height)]
    ...
    def gen_p(self, phi: Phi, p: int, q: int):
        ...
        temp_p = [[0. for _ in range(self.height)] for _ in range(self.height)]
    ...
    def iterate(self):
        ...
        for i in range(self.height):
            for j in range(i + 1, self.height):
                if abs(self.a[i][j]) > abs(self.a[p][q]):
                    p, q = i, j
        self.zero_element(p, q)


if __name__ == "__main__":
    a = [[float(i) for i in range(j, j + 3 * 5, 3)] for j in range(1, 14, 3)]
    test = Jacobi(a)
